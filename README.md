...
| ---------- | ---------------- | --------- | --------- |
| C++        | тяжёлая логика   | 50–200    | 100 Mb    |
| C++        | средняя логика   | 1000–2000 | 100 Mb    |
| Nginx      | SSL handshakes   | 500–5000  | 20 Mb     |
| C++        | лёгкое JSON API  | 2000      | 20 Mb     |
| Go         | лёгкое JSON API  | 4000      | 10–50 Mb  |
| Go         | средняя логика   | 200       | 50–100 Mb |
...
| Сервис              | Целевая пиковая нагрузка приложения                                      | Характер сервиса      | CPU  | RAM    | Net                            |
| ------------------- | ------------------------------------------------------------------------ | --------------------- | ---- | ------ | ------------------------------ |
| auth                | 213.2K `user` / 8 = 26.65K                                               | простая бизнес логика | 6    | 54 MB  | 148[^10] \* 26.65K = 30 MBit/s |
| tile render         | 95K `гео таблицы` / 8 = 11.9K RPS                                        | сложная бизнес логика | 1.2k | 116 GB | 670 MBit/s                     |
| routing service     | 95K `гео таблицы` / 8 = 11.9K RPS                                        | средняя бизнес логика | 119  | 1.2 GB | 483.4 MBit/s                   |
| map query service   | 70K `place_on_map` / 8 = 8.75K                                           | средняя бизнес логика | 119  | 8.5 GB | 68 Mbit/s                      |
| places data service | (\~70K `place` + 174K `review` + 213.2K `user`) / 8 = 457.2K / 8 = 57.2K | легкое JSON API       | 12   | 115 MB | 1.34 Gbit/s                    |

## Выбор модели развертывания

Будем использовать k8s для всех сервисов.
...
| tile render         | self-hosted | 2xEPYC 9474F/256GB/2x2TB NVMe/1x1Gbps     |  7  | \***\*45 000 ₽/мес** (Аморт.) |
