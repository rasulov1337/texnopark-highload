...
| **Таблица**         | **Описание**                                                                                     | Размер таблицы (байт) | Число записей                       | Объем   |
| ------------------- | ------------------------------------------------------------------------------------------------ | --------------------- | ----------------------------------- | ------- |
...
| **review**          | Отзывы пользователей о местах (текст, рейтинг)                                                   | 152                   | число мест \* 5K отзывов = 1 250 B  | 172 TB  |
...
-   Для ускорения поиска объектов в определенной области создадим пространственные индексы для таблиц `node`, `way` и `relation` по `latitude` & `longitude`
-   Для таблиц `node_tag`, `way_tag`, `relation_tag` создадим индексы по FK `node_id`, `way_id` и `relation_id` соответственно
...
Так как таблицы содержат мало полей, то имеет смысл отбросить вертикальное шардирование и применять горизонтальное.
Cassandra автоматически применяет шардирование, поэтому имеет смысл применить шардирование к БД, хранящихся под управлением PostgreSQL:

-   `node` & `node_tag` по `node_id`
-   `way` & `way_node` по `way_id`
