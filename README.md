...
| ------------------------ | --------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------- | -------- | ------------------ |
| **Информация о местах**  | 0,003 МБ                                                                                            | 2,5 (действий в сутки) / 86400 (секунд в сутках) \* 300M DAU ≈ `8.7K` | **18K**  | **410 Мбит/с**     |
| **Поиск мест**           | 50 байт (название организации) \* 10 результатов = 0,5 КБ                                           | 2,37 / 86400 \* 300M ≈ `8.2K`                                         | **18K**  | **64 Мбит/с**      |
| **Построение маршрутов** | Возьмем маршрут в 10 км: возможно 200 точек \* 16 байт = 3,2 КБ + метаданные маршрута 2 КБ = 5,2 КБ | 1,25 / 86400 \* 300M  ≈ `4.3K`                                        | **10K**  | **350 Мбит/с**     |
| **Отображение карты**    | при экране 1080p при масштабировании 16 уровня загружается 9-12 тайлов за раз. 12 \* 0,6 = 7,2 КБ   | 2,5 / 86400 \* 300M ≈ `8.7K`                                          | **18K**  | **10 Гбит/с**      |
...
| --------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| `node`, `node_tag`, `way`, `way_node`, `relation`, `relation_tag`, `relation_member`, `member_type` | (1,67M \* 70%) Информация о местах + <br>(1,58M) Поиск мест + <br>(0,83M) Построение маршрутов +<br>(1,67M \* 25%) Отображение карты = 4,5M | `100M` обновлений карты в день[^1][^16] / `86 400` (секунд в дне) = `1 157` QPS |
