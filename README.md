...
Возьмем распределение пользователей по странам с сайта hypestat[^10] и Sensor Tower[^21][^22][^23]:
...
| Тип запроса                        | Нью-Йорк | Лос-Анджелес | Чикаго | Франкфурт-на-Майне | Мумбаи | Пекин | Сан-Паулу | Токио |
| ---------------------------------- | -------- | ------------ | ------ | ------------------ | ------ | ----- | --------- | ----- |
| **Отображение карты** (34.8K)      | 1.93K    | 1.93K        | 1.93K  | 5.8K               | 5.8K   | 5.8K  | 5.8K      | 5.8K  |
| **Информации о местах** (34.8K)    | 1.93K    | 1.93K        | 1.93K  | 5.8K               | 5.8K   | 5.8K  | 5.8K      | 5.8K  |
| **Поиск мест и организаций** (33K) | 1.83K    | 1.83K        | 1.83K  | 5.5K               | 5.5K   | 5.5K  | 5.5K      | 5.5K  |
| **Построение маршрутов** (17.4K)   | 0.97K    | 0.97K        | 0.97K  | 2.9K               | 2.9K   | 2.9K  | 2.9K      | 2.9K  |
...
## Размеры таблиц после денормализации

| **Таблица**         | **Описание**                                                                                     | Размер записи (байт) | Число записей                       | Объем   |
| ------------------- | ------------------------------------------------------------------------------------------------ | -------------------- | ----------------------------------- | ------- |
| **place**           | Описывает места (название, описание, рейтинг, контакты, категория и т. д.)                       | 679                  | 250M[^2]                            | 158 GB  |
| **review**          | Отзывы пользователей о местах (текст, рейтинг)                                                   | 152                  | число мест \* 5 отзывов = 1.25 B    | 177 GB  |
| **user**            | Информация о пользователях (ID, имя, email, хеш пароля)                                          | 364                  | 2B                                  | 678 GB  |
| **node**            | Представляет географические точки (широта, долгота)                                              | 48                   | 9B[^13]                             | 402 GB  |
| **node_tag**        | Дополнительные атрибуты узлов (например, название улицы, высота и т. д.)                         | 232                  | 906M[^13]                           | 196 GB  |
...
| **relation**        | Определяет сложные связи между объектами (например, маршруты, границы)                           | 16                   | 13M[^13]                            | 198 MB  |
| **relation_tag**    | Дополнительные свойства для связей                                                               | 232                  | 53M[^13]                            | 11.5 GB |
| **relation_member** | Описывает участников отношений (узлы, пути, другие связи)                                        | 62                   | Число relation \* 7 members = 91M   | 5.25 GB |
| **tile_cache**      | Информация о кэше                                                                                | 44                   | 1.6B[^8]                            | 65.6 GB |
...
1. Рендеринг тайлов через Mapnik

2. Упаковка в формат .mbtiles с помощью `mb-util`
...
| ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| nginx        | \* rate limit<br/>\* балансировка нагрузки                                                                                                                                                                                                                                                                                                                                   |
| k8s          | \* Автоматический перезапуск контейнеров в случае сбоя<br/>\* Автоматическое масштабирование контейнеров                                                                                                                                                                                                                                                                     |
| Cassandra    | \* Шардирование<br/>\* Встроенная балансировка запросов на уровне драйвера                                                                                                                                                                                                                                                                                                   |
| PostgreSQL   | \* Резервное копирование<br/>\* Каскадные реплики<br/>\* Резервный Master в каждом ЦОДе. (Главный master в Европе. С него остальные резервные master-реплики делают репликацию. Если в Европе master отвалился, используем master, который находится в США. Если в США юзаем мастер в Китае и так далее.)<br/>\* Шардирование<br/>\* Асинхронная репликация<br/>\* pgBouncer |
| Тайловый кэш | \* Независимые кэширующие ноды в каждом регионе                                                                                                                                                                                                                                                                                                                              |
...
![Project Scheme](assets/overall.drawio.svg)

| Сервис              | Описание                                                       |
| ------------------- | -------------------------------------------------------------- |
| Routing Service     | Построение маршрутов                                           |
| Map Query Service   | Поиск мест и организаций                                       |
| Auth Service        | Авторизация и аутентификация                                   |
| Places Data Service | Сервис, который предоставляет данные о местах и их отзывах     |
| Tile Render Service | Рендеринг тайлов и объединение данных для тайлов в единый файл |

# 11. Список серверов

### **Конфигурации технологий**

| Технология     | Характер сервиса   | RPS (на ядро) | RAM (на ядро) |
| -------------- | ------------------ | ------------- | ------------- |
| **NGINX**      | L7-балансировка    | 100           | 20 МБ         |
| **Go (gRPC)**  | Тяжелые вычисления | 300           | 200 МБ        |
| **Go (CRUD)**  | Метаданные         | 5 000         | 50 МБ         |
| **PostgreSQL** | Геоданные          | 1 000         | 128 МБ        |
| **Cassandra**  | Метаданные         | 2 000         | 64 МБ         |

### **Сервисы и распределение нагрузки**

| Сервис                  | Общий RPS         | RPS на ЦОД (8 шт.) | Сетевой трафик (пиковый) | Трафик на ЦОД |
| ----------------------- | ----------------- | ------------------ | ------------------------ | ------------- |
| **Auth Service**        | 213.2K            | 26.65K             | 30 Мбит/с                | 3.75 Мбит/с   |
| **Routing Service**     | 94K (гео-таблицы) | 11.75K             | 707 Мбит/с               | 88.4 Мбит/с   |
| **Tile Render Service** | 34.8K             | 4.35K              | 19 Гбит/с                | 2.4 Гбит/с    |
| **Places Data Service** | 67.8K (места)     | 8.5K               | 19 Гбит/с                | 2.4 Гбит/с    |
| **Map Query Service**   | 8.5K (запросы)    | 1.06K              | 2.4 Гбит/с               | 0.3 Гбит/с    |

### **Расчет ресурсов для сервисов (на ЦОД)**

| Сервис                  | Технология      | RPS    | CPU (ядра)         | RAM                 | Сеть (Гбит/с) |
| ----------------------- | --------------- | ------ | ------------------ | ------------------- | ------------- |
| **Auth Service**        | Go (gRPC)       | 26.65K | 26.65K / 5 000 ≈ 6 | 6 × 50 МБ = 300 МБ  | 3.75          |
| **Routing Service**     | Go (gRPC)       | 11.75K | 11.75K / 300 ≈ 40  | 40 × 200 МБ = 8 ГБ  | 88.4          |
| **Tile Render Service** | Go (gRPC)       | 4.35K  | 4.35K / 300 ≈ 15   | 15 × 200 МБ = 3 ГБ  | 2.4           |
| **Places Data Service** | Go (CRUD)       | 8.5K   | 8.5K / 5 000 ≈ 2   | 2 × 50 МБ = 100 МБ  | 2.4           |
| **Map Query Service**   | Go + PostgreSQL | 1.06K  | 1.06K / 1 000 ≈ 2  | 2 × 128 МБ = 256 МБ | 0.3           |
| **NGINX**               | L7-балансировка | 4.35K  | 4.35K / 10 000 ≈ 1 | 1 × 20 МБ = 20 МБ   | 2.4           |

### **Конфигурация железа (self-hosted)**

| Компонент                   | Конфигурация оборудования                           | Кол-во | Стоимость (амортизация) |
| --------------------------- | --------------------------------------------------- | ------ | ----------------------- |
| **Серверы Auth**            | 2xEPYC 7B12/32x64GB/4xNVMe8T/4x25Gb/s               | 8      | $4 800/мес              |
| **Серверы Routing**         | 8xEPYC 9654/64x512GB/16xNVMe16T/8x200Gb/s           | 12     | $28 800/мес             |
| **Серверы Tile Render**     | 2xEPYC 9554/64x256GB/2xNVIDIA T4/8xNVMe16T          | 8      | $6 400/мес              |
| **Серверы Places Data**     | 2xXeon Gold 6448Y/32x128GB/6xSSD8T/4x25Gb/s         | 8      | $6 400/мес              |
| **Серверы Map Query**       | 2xXeon E-2388G/16x64GB/2xNVMe4T/2x10Gb/s            | 8      | $1 600/мес              |
| **PostgreSQL (геоданные)**  | 4xXeon Platinum 8490H/128x512GB/12xSSD16T/4x100Gb/s | 3      | $4 800/мес              |
| **Cassandra (метаданные)**  | 2xXeon Silver 4510/48x256GB/8xSSD32T/4x25Gb/s       | 16     | $9 600/мес              |
| **NGINX (L7-балансировка)** | 2xXeon E-2378G/16x64GB/2xNVMe2T/2x10Gb/s            | 8      | $1 600/мес              |

### K8s

| Сервис              | CPU/r | CPU/l | RAM/r  | RAM/l  | Cnt | Обоснование                                                                 |
| ------------------- | ----- | ----- | ------ | ------ | --- | --------------------------------------------------------------------------- |
| **Auth Service**    | 4     | 6     | 0.5 Gb | 1 Gb   | 6   | 26.65K RPS. Запросы: 4 ядра, 0.5 ГБ. Лимиты: 6 ядер, 1 ГБ. 6 реплик для HA. |
| **Routing Service** | 40    | 48    | 220 Gb | 256 Gb | 10  | Алгоритмы маршрутизации требуют загрузки графа (174.5 ГБ) + кэш (50 ГБ).    |
| **Tile Render**     | 60    | 68    | 3.5 Gb | 4 Gb   | 8   | Mapnik требует загрузки стилей (2 ГБ) + данных (1 ГБ) + кэша (0.5 ГБ).      |
| **Places Data**     | 1     | 2     | 0.1 Gb | 0.2 Gb | 4   | 8.5K RPS. Легкая CRUD-нагрузка. 4 реплики для балансировки.                 |
| **Map Query**       | 1     | 2     | 0.2 Gb | 0.3 Gb | 4   | 1.06K RPS. Умеренные запросы. 4 реплики для резервирования.                 |
...
[^21]: [Sensor Tower: Google Maps Go](https://app.sensortower.com/overview/com.google.android.apps.mapslite?country=US#:~:text=Top%20Countries%20/%20Regions)
[^22]: [Sensor Tower: Google Maps Android](https://app.sensortower.com/overview/com.google.android.apps.maps?country=US#:~:text=Top%20Countries%20/%20Regions)
[^23]: [Sensor Tower: Google Maps iOS](https://app.sensortower.com/overview/585027354?country=US#:~:text=Top%20Countries%20/%20Regions)
