...
- Для таблиц `node`, `node_tag`, `way`, `way_node`, `relation`, `relation_tag`, `relation_member`, `member_type` будем использовать 50 реплик на чтение. Воспользуемся каскадной репликацией, чтобы избежать нагрузки на master.
...
![Project Scheme](assets/Scheme.drawio.png)
...
| Go         | легкое JSON API       | 4000 | 10-50 Mb  |
| Go         | средняя бизнес-логика | 100  | 50-100 Mb |
