...
| -------------- | ------------------ | ------------- | ------------- |
| **NGINX**      | L7-балансировка    | 100           | 50 МБ         |
| **NGINX**      | CDN Cache          | 2 000         | 50 MБ         |
| **Go (gRPC)**  | Тяжелые вычисления | 200           | 300 МБ        |
| **Go (CRUD)**  | Метаданные         | 3 500         | 100 МБ        |
| **PostgreSQL** | Геоданные          | 100           | 18 ГБ         |
| **Cassandra**  | Метаданные         | 1 400         | 6 ГБ          |

### **Сервисы и распределение нагрузки**

| Сервис                  | Общий RPS      | RPS на ЦОД (8 шт.) | Сетевой трафик (пиковый) | Трафик на ЦОД |
| ----------------------- | -------------- | ------------------ | ------------------------ | ------------- |
| **Auth Service**        | 643 + 80 = 723 | 91                 | 565 Кбит/с               | 70.6 Кбит/с   |
| **Routing Service**     | 17.4K          | 2.175K             | 707 Мбит/с               | 88.4 Мбит/с   |
| **Tile Render Service** | 34.8K          | 4.35K              | 19 Гбит/с                | 2.4 Гбит/с    |
| **Places Data Service** | 34.8K          | 4.35K              | 835 Мбит/с               | 104.4 Мбит/с  |
| **Map Query Service**   | 33K            | 4.125K             | 258 Мбит/с               | 32.3 Мбит/с   |

### **Расчет ресурсов для сервисов (на ЦОД)**

| Сервис                  | Технология      | RPS                         | CPU (ядра)        | RAM                  | Сеть         |
| ----------------------- | --------------- | --------------------------- | ----------------- | -------------------- | ------------ |
| **Auth Service**        | Go (gRPC)       | 91                          | 91 / 200 ≈ 1      | 1 × 300 МБ = 300 МБ  | 70.6 Кбит/с  |
| **Routing Service**     | Go (gRPC)       | 2.175K                      | 2.175K / 200 ≈ 11 | 11 × 300 МБ = 3.2 ГБ | 88.4 Мбит/с  |
| **Tile Render Service** | Go (gRPC)       | 4.35K                       | 4.35K / 100 ≈ 44  | 44 × 300 МБ = 13 ГБ  | 2.4 Гбит/с   |
| **Places Data Service** | Go (CRUD)       | 4.35K                       | 8.5K / 3 500 ≈ 2  | 2 × 100 МB = 200 МB  | 104.4 Мбит/с |
| **Map Query Service**   | Go (CRUD)       | 4.125K                      | 1.06K / 3 500 ≈ 2 | 2 × 200 МB = 200 МB  | 32.3 Мбит/с  |
| **NGINX**               | L7-балансировка | 10.8K                       | 10.8K / 100 ≈ 108 | 108 × 50 МБ = 5.3 ГБ | 225.2 Мбит/с |
| **CDN Cache**           | Отдача статики  | 4.35K (Tile Render Service) | 4.35K / 200 ≈ 3   | 3 × 50 МB = 150 MB   | 2.4 Гбит/с   |

### K8s

| Сервис              | CPU/r | CPU/l | RAM/r  | RAM/l  | Cnt |
| ------------------- | ----- | ----- | ------ | ------ | --- |
| **Auth Service**    | 1     | 2     | 0.3 Gb | 0.6 Gb | 6   |
| **Routing Service** | 40    | 48    | 220 Gb | 256 Gb | 10  |
| **Tile Render**     | 22    | 24    | 6.6 Gb | 7.2 Gb | 8   |
| **Places Data**     | 2     | 3     | 0.2 Gb | 0.3 Gb | 4   |
| **Map Query**       | 2     | 3     | 0.2 Gb | 0.3 Gb | 4   |

### **Конфигурация железа (self-hosted)**

| Компонент                  | Конфигурация оборудования                                 | Кол-во | Стоимость (амортизация) |
| -------------------------- | --------------------------------------------------------- | ------ | ----------------------- |
| **Серверы Auth**           | 2×EPYC 9554 / 8×64 GB RAM / 2×2 TB NVMe / 2×10 Gb/s       | 4      | \~$5 000/мес            |
| **PostgreSQL (геоданные)** | 3×EPYC 7543 / 24×128 GB RAM / 4×4 TB NVMe / 1×10 Gb/s     | 3      | $1 200/мес              |
| **Cassandra (метаданные)** | 5×Xeon Gold 5318N / 12×64 GB RAM / 4×8 TB SSD / 1×10 Gb/s | 5      | $800/мес                |
| **NGINX**                  | 2×Xeon E‑2378G / 4×16 GB RAM / 1×1 TB NVMe / 1×10 Gb/s    | 2      | $200/мес                |
